...
def file(mm, bp, logname, version, compute_bond_dists, N0, dist_orient):
    ...
    with open(logname, 'w') as f:
        ...
        #################################
        # Write Reference molecule info #
        #################################
        def adjustangle(angle):
            # Find true oreitnation based on max_diff.
            max_diff = 10 # degrees
            if angle > 0:
                if abs(angle - 90) <= max_diff: angle = angle - 90
                if abs(angle - 180) <= max_diff: angle = angle - 180
                if abs(angle - 270) <= max_diff: angle = angle - 270
            if angle < 0:
                if abs(angle + 90) <= max_diff: angle = angle + 90
                if abs(angle + 180) <= max_diff: angle = angle + 180
                if abs(angle + 270) <= max_diff: angle = angle + 270
        ...
        if dist_orient['perform']:
            f.write('\n\n{}\n\n'.format(str(dist_orient)))
            f.write('\n\n--------------------------------------Reference Molecule(s)-------------------------------------\n')
            f.write('{:<6} {:^20} {:>8} {:>8} {:>8} {:>12} {:>12} {:>12}\n'.format('molID', 'formula', 'X-angle', 'Y-angle', 'Z-angle', 'X-center', 'Y-center', 'Z-center'))
            f.write('------------------------------------------------------------------------------------------------\n')
            for ID_ref in mm.ref_molecules:
                mol = mm.ref_molecules[ID_ref]
                anglex, angley, anglez = mol.angles
                xcenter, ycenter, zcenter = mol.center
                f.write('{:<6} {:^20} {:>8.2f} {:>8.2f} {:>8.2f} {:>12.2f} {:>12.2f} {:>12.2f}\n'.format(ID_ref, mol.formula, adjustangle(anglex), adjustangle(angley),
                                                                                                  adjustangle(anglez), xcenter, ycenter, zcenter))
...
# Function to write log file
def csv(m, name, version):
    # write file
    with open(name, 'w') as f:
        # Write headers
        headers = ['Reference molID', 'Reference formula', 'Test molID', 'Test formula', 'Perpendicular Distance', 'Planar Angle (Deg)',
                   'Relative X-angle (Deg)', 'Relative Y-angle (Deg)', 'Relative Z-angle (Deg)', 'Reference X-center',
                   'Reference Y-center', 'Reference Z-center', 'Test X-center', 'Test Y-center', 'Test Z-center']
        f.write('{}\n'.format(', '.join(headers)))
        ...
        for i in m.dist_orientation_molecules:
            ...
            id1 = str(i[0])
            id2 = str(i[1])
            formula1 = m.datatable[i[0]].formula
            formula2 = m.datatable[i[1]].formula
            distance = '{:.2f}'.format(dist)
            plane_angle = '{:.2f}'.format(plane_angle)
            anglex = '{:.2f}'.format(angles[0])
            angley = '{:.2f}'.format(angles[1])
            anglez = '{:.2f}'.format(angles[2])
            refx, refy, refz = m.datatable[i[0]].center
            refx = '{:.2f}'.format(refx)
            refy = '{:.2f}'.format(refy)
            refz = '{:.2f}'.format(refz)
            testx, testy, testz = m.datatable[i[1]].center
            testx = '{:.2f}'.format(testx)
            testy = '{:.2f}'.format(testy)
            testz = '{:.2f}'.format(testz)
            data = [id1, formula1, id2, formula2, distance, plane_angle, anglex,
                    angley, anglez, refx, refy, refz, testx, testy, testz]
            f.write('{}\n'.format(', '.join(data)))
