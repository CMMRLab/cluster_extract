...
Revision 2.0 (2.0 no longer uses Jake's ring code to find cycles')
May 24th, 2023
...
class fused_rings:
    def __init__(self, m, find_rings, ringIDs):
        ...
        #-----------------------------------------#
        clusters = [set([ID]) for ID in ringIDs] # [ lst of sets to find fused clusters ]
        for id1, id2 in self.bonded:
            for c1, cluster in enumerate(clusters):
                if id1 in cluster: break
            for c2, cluster in enumerate(clusters):
                if id2 in cluster: break
            if c1 != c2:
              clusters[c1].update(clusters[c2])
              del clusters[c2]
        clusters = [tuple(sorted(i)) for i in clusters] # Sort each individual cluster ringIDs in ascending order
        ...
        def find_bonds_in_fused_cluster(cluster):
            ...
            for ringID in cluster:
                ...
                for i in m.bonds:
                    id1, id2 = m.bonds[i].atomids
                    if id1 in atoms and id2 in ringIDs[ringID]:
...
class ring_analysis:
    def __init__(self, m, find_rings):
        ...
        self.cycles = find_cycles(m, find_rings)
        ...
        if find_rings['fused-rings']: self.fused = fused_rings(m, find_rings, self.ringIDs)
...
#####################################################
def add_ring_data2m(m, find_rings):
    
    # Call rings class from above that will call other auxiliary ring class
    rings_data = ring_analysis(m, find_rings)
