...
###########################################
def interatomic_brute_force(m, maxdist_dict, radial, edgeflags, scaled_images):
    ...
    print(f'    Bond generation execution time: {execution_time} (seconds)')
    return possible_bonds, bond_status 

################################################
# cell linked list interatomic distance search #
################################################
def interatomic_cell_linked(m, maxdist_dict, radial, edgeflags, scaled_images, xlo, xhi, ylo, yhi, zlo, zhi):
    ...
    print('Finding cell linked graph for interatomic distance calculations ...')
    for id1 in domain:
        d1 = domain[id1]
    ...
    possible_bonds = {} # { tuple(id1, id2): distance_fff or distance_ppp }
    bond_status = {'periodic': 0, 'non-periodic': 0} # to tally bond types
    print('Finding interatomic distances for bond creation (cell linked list) ....')
    progress_increment = 10; count = 0; natoms = len(m.atoms);
...
class generate:
    def __init__(self, m, boundary, vdw_radius_scale, maxbonded):
        ...
        print('\n---------------------------------------')
        print('System Unit cell, volume, mass, density')
        print('---------------------------------------')
        print('{:<10} {:^10.5f} {:<10}'.format('Lx:', lx, 'angstrom'))
        print('{:<10} {:^10.5f} {:<10}'.format('Ly:', ly, 'angstrom'))
        print('{:<10} {:^10.5f} {:<10}'.format('Lz:', lz, 'angstrom'))
        print('{:<10} {:^10.4E} {:<10} {}'.format('volume:', volume, 'cm^3', '(assumes orthogonality)'))
        print('{:<10} {:^10.4E} {:<10}'.format('mass:', mass, 'grams'))
        print('{:<10} {:^10.5f} {:<10} {}' .format('density:', density, 'g/cm^3', '(assumes orthogonality)'))
        ...
        if len(m.atoms) < 7500 or density < 0.25 or lx < 18 or ly < 18 or lz < 18:
            possible_bonds, self.bond_status = interatomic_brute_force(m, maxdist_dict, radial, edgeflags, scaled_images)
            ...
            try: possible_bonds, self.bond_status = interatomic_cell_linked(m, maxdist_dict, radial, edgeflags, scaled_images, xlo, xhi, ylo, yhi, zlo, zhi)
            except:
                print('Cell linked list method failed, resorting to brute force method')
                possible_bonds, self.bond_status = interatomic_brute_force(m, maxdist_dict, radial, edgeflags, scaled_images)
