...
delete_atoms = {'method': 'mass', # 'size' or 'mass'
                'criteria': 100 }   # if 'size' natoms: if 'mass' mass in amu
...
#-------------------------------------------------------------------------------------------------------------#
compute_bond_dists = {'kept': False,            # Option to find bond length of kept clusters
                      'hybridization': False,   # Option to reset bond types based on element and hybridization
                      'removed': False}         # Option to find bond length of removed clusters
...
#------------------------------------------------------------------------------------------------------------#
parent_directory = 'Examples/auto_logging1'


#------------------------------------------------------------------------------------------------------------#
# Auto logging / LAMMPS analysis / LAMMPS deleting specific settings. Such as log file name and intervals ...#
#------------------------------------------------------------------------------------------------------------#
logfile = 'cluster_extract_auto_logging_test' # cluster extracts automated logging name
time = 0        # will be used to tally time
interval = 105  # is the time interval that gets tallied
maxtime = 515 # Sets the maximum time of files
...
if __name__ == "__main__":  
    ...
    # Find present working directory and join with parent_directory
    pwd = os.getcwd(); path = os.path.join(pwd, parent_directory) # Find/create paths to store code results
    if not os.path.isdir(path): os.mkdir(path) # Check if path exists. IF not create.
    
    # Set Filenames with correct paths
    convergencefile = os.path.join(path, logfile+'.csv')
    byproductsfile = os.path.join(path, logfile+'.del')
    ...
    for n in range(niterations):
        ...
        topofile = 'Examples/auto_logging1/post_poly_rep1_' + str(time) + 'ps.data'
        bondfile = 'Examples/auto_logging1/post_poly_rep1_' + str(time) + 'ps.reaxc'
        ...
        # run auto logger for convergence data and by-products data
